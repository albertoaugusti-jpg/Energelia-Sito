...
RESEND_FROM = os.environ.get("RESEND_FROM", "CRM Energelia <noreply@energelia.it>")
...
def invia_email(destinatario, oggetto, corpo, rispondi_a=None, nome_mittente="Energelia"):
    ...
    if RESEND_API_KEY:
        try:
            payload = {
                "from": RESEND_FROM or f"{nome_mittente} <noreply@energelia.it>",
                "to": [destinatario],
                "subject": oggetto,
                "text": corpo,
            }
            if rispondi_a:
                payload["reply_to"] = rispondi_a
            r = requests.post(
    ...
    if not (SMTP_USER and SMTP_PASS):
        return False, "L'invio email non è configurato: imposta RESEND_API_KEY su Render."
    try:
        msg = MIMEMultipart()
